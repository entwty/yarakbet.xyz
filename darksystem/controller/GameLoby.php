...
class GameLoby extends controller {
    ...
    public function Slot() {
        ...
        $casino_banners = $this->db->result("select * from casino_banners where live = 0 order by `index` ASC");
        $casino_banners = json_decode(json_encode($casino_banners));



        $casino_games = $this->db->result("select * from casino_games where live = 0 and active = 1  order by id DESC");
        $casino_games = json_decode(json_encode($casino_games));
    ...
    public function Token($id){
        ...
        if (empty($find_casino)) { die("Casino Bulunamad覺"); }
        ...
        if (empty($bilgi)) {
            ...
            $response = httpPost("http://apicdn.betabahis.tech",$data);
            $response = json_decode($response);
            echo $response;
        } else {

                $data = array("method" => "token","user_id"=> $userid, "username" => $username, "name" => $name,"game_id"=>$table_id);
                $response = httpPost("https://games.gapi.lol/",$data);
                $response = json_decode($response);
                $url ="https://games.gapi.lol/".$userid."&game_id=".$table_id;
