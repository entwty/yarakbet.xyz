...
class services extends controller {
  ...
  function matchplus($id) {
    ...
    $db = $this->db; // veya global $db; if you are using a global connection

    // betsapitechkey'i al
    $sql = "SELECT betsapitechkey FROM odd_services LIMIT 1"; // LIMIT 1 sadece ilk satırı alır
    $result = $db->result($sql);

    // Veritabanı sorgu sonucu kontrolü
    if (!$result || count($result) === 0) {
        return "Anahtar bulunamadı.";
    }

    // Anahtar bulundu
    $apiKey = $result[0]['betsapitechkey'];

    // API URL'yi oluştur
    $url = "https://betsapi.tech/api/presportsodds.php?token=" . $apiKey . "&matchid=" . $id;

    // API'den veri al
    $jsonData = file_get_contents($url);

    // Hata kontrolü: API'den veri alınıp alınamadığını kontrol et
    if ($jsonData === FALSE) {
        return "API'den veri alınamadı.";
    }

    // JSON verisini decode et
    $getList = json_decode($jsonData);

    // JSON decode hatası kontrolü
    if (json_last_error() !== JSON_ERROR_NONE) {
        return "JSON decode hatası: " . json_last_error_msg();
    }
    ...
    // Spor türünü ve ev sahibi, misafir takımların adlarını al
    ...
    switch ($sport) {
        case 'FOOTBALL': $sportid = 0; break;
        case 'BASKETBALL': $sportid = 12; break;
        case 'VOLLEYBALL': $sportid = 19; break;
        case 'TENNIS': $sportid = 4; break;
        case 'BASEBALL': $sportid = 13; break;
        case 'TABLE_TENNIS': $sportid = 34; break;
        default: $sportid = 9999; break;
    }

    // Durum değerini belirle
    $state = $getList->events[0]->state;
    $state = ($state === 'STARTED') ? 1 : 0;

    // Event bilgilerini XML'e ekle
