        ...
        if ($miktar) {
            ...
            foreach ($kupon as $k => $za) {
                ...
                if ($m["canlimi"] == "1") {
                    ...
                    $betxRadarId = $kaynak->LiveEvents->E->attributes()->BetRadarID[0];
                    ...
                    foreach($ilkbu as $ilk){
                        ...
                        foreach($ilk->R as $val){
                            if ($ilk->attributes()->N == $grup && $val->attributes()->N ==  $za->orantur) {
                                ...
                                if ($ilk->attributes()->GameIsVisible == 0 or $val->attributes()->O0<"1.01") {
                                    die("Oynadığınız Maçın Oranı Askıda veya Geçersiz Oran");
                                    $oranimbu = "1";
                                } else {
                                    //İkinci kontrol
                                    $kaynaks = $this->oddsVeri($m["botid"]);
                                    $ilkbus=$kaynaks->LiveEvents->E->Games->G;
                                    foreach($ilkbus as $ilks){
                                        $grups = $za->orangrup;
                                        foreach($ilks->R as $vals){
                                            if ($ilks->attributes()->N == $grups && $vals->attributes()->N ==  $za->orantur) {
...
//                                                }
                                                ...
                                                if ($ilks->attributes()->GameIsVisible == 0 or $vals->attributes()->O0<"1.01") {
                                                    die("Oynadığınız Maçın Oranı Askıda veya Geçersiz Oran");
                                                    $oranimbu = "1";
                                                }
                                                else {
    ...
    function DepositOk() {
        ...
        mysql_query("insert into parayatir (uye,adsoyad,banka,miktar,tur,bonus) values ('$user_id','$adisoyadi','$bankasi','$para','HAVALE','$bonus')");
        echo '{"vResult":"TRUE","vHeader":"Error","vContent":"Para Yatırma Talebiniz İşleme Alınmıştır!"}';
    }
    ...
    function PaparaDeposit() {
        ...
        mysql_query("insert into parayatir (uye,adsoyad,banka,miktar,tur,bonus) values ('$user_id','$adisoyadi','$bankasi','$para','PAPARA','$bonus')");
        echo '{"vResult":"TRUE","vHeader":"Error","vContent":"Para Yatırma Talebiniz İşleme Alınmıştır!"}';
    }
