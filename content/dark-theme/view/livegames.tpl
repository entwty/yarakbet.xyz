{php} include "content/dark-theme/view/dark_header.php";{/php}
<div class="navbar-title visible-sm visible-xs">
    <a class="page-title"><i class="icon-play3"></i>AVİATOR</a>
</div>

<section id="main" class="backgroundsize " >
    <iframe id="tvbet-iframe"
            style="width: 100%;"
            height="815px"
            src="https://demo.spribe.io/launch/aviator?currency=TRY&lang=TR&return_url=https://spribe.co/games";
            frameborder="0"
            scrolling="no"
            allowfullscreen
            allow="autoplay">
    </iframe>
</section>
{php} include "content/dark-theme/view/dark_footer.php";{/php}