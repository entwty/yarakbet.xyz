function popupwindow(url, title, w, h) {
    ...
    return window.open(url, title, 'directories=no,titlebar=no,toolbar=no,location=no,status=no,menubar=no,scrollbars=no,resizable=no,copyhistory=no, width=' + w + ', height=' + h + ', top=' + top + ', left=' + left);
}

var serviceUrl = "/services/";
function matchDetails(id, tis) {
    var before = $(tis).html();
    $(tis).html("<i class='fa fa-spinner fa-spin' style='display: block; color: #fff;padding-top: 5px;'></i>");
	...
	$.ajax({
		...
		oranhesapla();
            ...
            function bultenget(veri) {
                $('body').append('<div id="loadoverlay" style="position:fixed; top:0px; left:0px; bottom:0px; right:0px; width:100%; height:100%; background:url(/content/betting-theme/images/darkoverlay.png); text-align:center; padding-top:200px; z-index:10000;"><img alt="" src="/content/betting-theme/images/spinner.gif" /></div>');
...
function content_block(container) {
    $(container).append('<i class="fa fa-spinner fa-pulse content_loading_spinner"></i>');
}
...
function content_unblock(container) {
  ...
  $(document).on("click", "#match_list_sides_1", function (e) {
        ...
        $(this).parent('.match_list_row').children('.match_list_sides_2_content').mCustomScrollbar('destroy');
        if ($(this).parent('.match_list_row').children('.match_list_sides_2_content').is(":visible") == false) {
            $(this).parent('.match_list_row').children('.match_list_sides_2_content').slideDown(250);
            list_prematchdetail($(this).parent('.match_list_row').attr('data-MatchID'), $(this).parent('.match_list_row').attr('data-DataSource'));
        }
        else {
...
function list_prematchdetail(MatchID, DataSource) {
    ...
    $.ajax({
        ...
        success: function (data) {
                ...
                $('#match_' + MatchID).children('.match_list_sides_2_content').html(data);
                content_unblock($('#match_' + MatchID).children('.match_list_sides_2_content'));
				$(".match_list_sides_2_content").mCustomScrollbar({scrollInertia: 2000, mouseWheelPixels: 160});
...
function kazanchesapla(){
		...
		$('.makskazanc-rakam').html(addCommas(Math.round($('.oran-rakam').html()*$('.kmiktar').val()*100)/100)+ " TL");
...
function fnLoginCheck() {
    $.ajax({
        type: 'POST',
        data: 'username=' + $('#edtUsername').val() + '&password=' + $('#edtPassword').val(),
        url: '/services/login',
        success: function (data) {
...
function paparaDeposit() {
    ...
    $.ajax({
        type: 'POST',
        data: '&paparanamesurname=' + $('#paparanamesurname').val() + '&paparamoney=' + $('#paparamoney').val(),
        url: '/myaccount/PaparaDeposit?rand='+Math.random(),
        success: function (data) {
            var json = $.parseJSON(data);
            if (json.vResult == 'TRUE') {
                Ply.dialog("alert", "Para Yatırma Talebiniz,İncelenmek Üzere Kayıt Edilmiştir.");
                $('body').children('#loadoverlay').fadeOut(250);
                setTimeout(function () { $('body').children('#loadoverlay').remove(); }, 250);
            } else {
...
function pepleDeposit() {
    ...
    $.ajax({
        type: 'POST',
        data: '&peplenamesurname=' + $('#peplenamesurname').val() + '&peplemoney=' + $('#peplemoney').val(),
...
function bitcoinDeposit() {
    ...
    $.ajax({
        type: 'POST',
        data: '&bitcoinnamesurname=' + $('#bitcoinnamesurname').val() + '&bitcoinmoney=' + $('#bitcoinmoney').val(),
...
function tetherDeposit() {
    ...
    $.ajax({
        type: 'POST',
        data: '&tethernamesurname=' + $('#tethernamesurname').val() + '&tethermoney=' + $('#tethermoney').val(),
...
function mefeteDeposit() {
    ...
    $.ajax({
        type: 'POST',
        data: '&mefetenamesurname=' + $('#mefetenamesurname').val() + '&mefetemoney=' + $('#mefetemoney').val(),
...
function jetonDeposit() {
    ...
    $.ajax({
        type: 'POST',
        data: '&jetonnamesurname=' + $('#jetonnamesurname').val() + '&jetonmoney=' + $('#jetonmoney').val(),
...
function cmtDeposit() {
    ...
    $.ajax({
        type: 'POST',
        data: '&cmtnamesurname=' + $('#cmtnamesurname').val() + '&cmtmoney=' + $('#cmtmoney').val(),
...
function payfixDeposit() {
    ...
    $.ajax({
        type: 'POST',
        data: '&payfixnamesurname=' + $('#payfixnamesurname').val() + '&payfixmoney=' + $('#payfixmoney').val(),
...
function paraodeDeposit() {
    ...
    $.ajax({
        type: 'POST',
        data: '&paraodenamesurname=' + $('#paraodenamesurname').val() + '&paraodemoney=' + $('#paraodemoney').val(),
...
function bankDeposit() {
    ...
    $.ajax({
        type: 'POST',
        data: '&time=' + $('#time').val() + '&bank=' + $('#bank').val() + '&namesurname=' + $('#namesurname').val()+ '&money=' + $('#money').val(),
        url: '/myaccount/DepositOk?rand='+Math.random(),
        success: function (data) {
            var json = $.parseJSON(data);
            if (json.vResult == 'TRUE') {
				Ply.dialog("alert",{ effect: "3d-sign" }, "Para Yatırma Talebiniz,İncelenmek Üzere Kayıt Edilmiştir.");
			   $('body').children('#loadoverlay').fadeOut(250);
                setTimeout(function () { $('body').children('#loadoverlay').remove(); }, 250);
            } else {
				Ply.dialog("alert",{ effect: "3d-sign" }, json.vContent);
...
function qrDeposit() {
    ...
    $.ajax({
        ...
        success: function (data) {
            var json = $.parseJSON(data);
            if (json.vResult == 'TRUE') {
                $('#qrframe').html("<a href='" + json.vUrl + "' target='_blank' class='remodal-confirm' style='padding: 15px 20px 15px 20px;'>KOD TARATMAK İÇİN TIKLAYINIZ >> </a>");
...
function fnMobileBankOk() {
    ...
    $.ajax({
        type: 'POST',
        data: 'bonuscep=' + $('#bonuscep').val() +'&bank=' + $('#bank2').val() +'&note=' + $('#note').val() + '&gtel=' + $('#gtel').val()+ '&atel=' + $('#atel').val()+ '&atc=' + $('#atc').val()+ '&referans=' + $('#referans').val()+ '&money=' + $('#money2').val(),
...
function fnBkmExpress() {
    ...
    $.ajax({
        type: 'POST',
        data: 'bonuscep=' + 'BKM' +'&bank=' + $('#bkmBank').val() +'&note=' + $('#bkmNote').val() + '&gtel=' + $('#bkmGtel').val()+ '&atel=' + $('#bkmAtel').val()+ '&atc=' + $('#bkmAtc').val()+ '&referans=' + $('#bkmRef').val()+ '&money=' + $('#bkmMoney').val(),
        url: '/myaccount/MobileBankOk?rand='+Math.random(),
        success: function (data) {
            var json = $.parseJSON(data);
            if (json.vResult == 'TRUE') {
...
function fnCreditCard() {
    ...
    $.ajax({
        type: 'POST',
        data: '&creditTel=' + $('#creditTel').val()+ '&creditMoney=' + $('#creditMoney').val(),
...
function fnAstroPay() {
    ...
    $.ajax({
        type: 'POST',
        data: 'astroCardno=' + $('#astroCardno').val() +'&astroCardCode=' + $('#astroCardCode').val() + '&astroMonth=' + $('#astroMonth').val()+ '&astroYear=' + $('#astroYear').val()+ '&astroMoney=' + $('#astroMoney').val(),
...
function fnPepleWithdraw() {
    ...
    $.ajax({
        type: 'POST',
        data: 'cardnopeple=' + $('#cardnopeple').val() +'&quantitypeple=' + $('#quantitypeple').val(),
...
function fnbtcWithdraw() {
    ...
    $.ajax({
        type: 'POST',
        data: 'btcno=' + $('#btcno').val() +'&quantitybtc=' + $('#quantitybtc').val(),
...
function fntetherWithdraw() {
    ...
    $.ajax({
        type: 'POST',
        data: 'tetherno=' + $('#tetherno').val() +'&quantitytether=' + $('#quantitytether').val(),
...
function fncmtWithdraw() {
    ...
    $.ajax({
        type: 'POST',
        data: 'cardnocmt=' + $('#cardnocmt').val() +'&quantitycmt=' + $('#quantitycmt').val(),
...
function fnpayfixWithdraw() {
    ...
    $.ajax({
        type: 'POST',
        data: 'cardnopayfix=' + $('#cardnopayfix').val() +'&quantitypayfix=' + $('#quantitypayfix').val(),
...
function fnProfileUpdate() {
    ...
    $.ajax({
        type: 'POST',
        data: 'old_password=' + $('#old_password').val() + '&new_password=' + $('#new_password').val()+ '&new_password_repeat=' + $('#new_password_repeat').val(),
...
function fnPokerPasswordChange() {
    ...
    $.ajax({
        type: 'POST',
        data: 'poker_password=' + $('#poker_password').val() + '&poker_password_repeat=' + $('#poker_password_repeat').val(),
...
function StatsDetails(id) {
    var url = "https://sportcenter.sir.sportradar.com/tr/match/" + id;
...
function fnBitcoin() {
    ...
    $.ajax({
        ...
        success: function (data) {
            var json = $.parseJSON(data);
            if (json.vResult == 'TRUE') {
                $('#btciframe').html("<iframe  id='frame' name='frame' src='" + json.vUrl + "' width='600'  height='600'   allowfullscreen></iframe>");
...
function fnWithDrawOk() {
    ...
    $.ajax({
        type: 'POST',
        data: 'bank=' + $('#bank').val() + '&sube=' + $('#sube').val()+ '&hesap=' + $('#hesap').val()+ '&iban=' + $('#iban').val()+ '&money=' + $('#money').val(),
        url: '/myaccount/WithDawOk?rand='+Math.random(),
        success: function (data) {
            var json = $.parseJSON(data);
            if (json.vResult == 'TRUE') {
				Ply.dialog("alert",{ effect: "3d-sign" }, "Çekim İşleminiz Başarıyla Kayıt Edildi");
			   $('body').children('#loadoverlay').fadeOut(250);
                setTimeout(function () { $('body').children('#loadoverlay').remove(); }, 250);
				fnGetBalance();
            } else {            
...
function fnWithDraw2Ok() {
    ...
    $.ajax({
        type: 'POST',
        data: 'bank2=' + $('#bank2').val() + '&sube2=' + $('#sube2').val()+ '&hesap2=' + $('#hesap2').val()+ '&iban2=' + $('#iban2').val()+ '&money2=' + $('#money2').val(),
...
function fnPreListByDefault(slider_hour) {
    ...
    if (hour > 12) hour = 12;
    $('body').append('<div id="loadoverlay" style="position:fixed; top:0px; left:0px; bottom:0px; right:0px; width:100%; height:100%; background:url(/images/darkoverlay.png); text-align:center; padding-top:200px; z-index:10000;"><img alt="" src="/images/spinner.gif" /></div>');
    $.ajax({
        type: 'POST',
        data: 'hour=' + hour,
...
function fnPreListByTournament(TournamentID) {
    ...
    $.ajax({
        type: 'POST',
        data: 'hour=' + $('#slider').attr('data-hour') + '&TournamentID=' + TournamentID,
...
function fnPreListByCategory(CategoryID, tis) {
		...
		if ( $(tis).hasClass("active") ) {
			...
			$('#content').html('<div align="center"><div class="loader" style="color:#000;">Loading...</div></div>');
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (json.vResult == 'TRUE') {
                ...
                $(".coupon_sil[data-couponid=" + couponid + "]").parent('td').parent('tr').children('td').addClass('deleted_coupon');
                $(".coupon_sil[data-couponid=" + couponid + "]").parent('td').parent('tr').children('td').eq(7).html('');
                fnGetBalance();
            }
            else {
                toastr.options = { "progressBar": true, "closeButton": true, "positionClass": "toast-top-right" };
...
function fnAddPreMatch(OddsID, Odds, OddsOrder, DataSource) {
    $.ajax({
        type: 'POST',
        data: 'OddsID=' + OddsID + '&Odds=' + Odds + '&OddsOrder=' + OddsOrder + '&DataSource=' + DataSource,
        url: '/addprematch',
        success: function (data) {
            var json = $.parseJSON(data);
            if (json.vResult == 'TRUE') {
...
function fnCalculatePayout() {
    var checkboxvalues = $('input[name="edtSystem"]:checked').map(function () { return this.value; }).get();
    $.ajax({
        type: 'POST',
        data: 'amount=' + $('#edtAmount').val().replace(/\./g, '').replace(',', '.') + '&system=' + checkboxvalues.join(','),
        url: '/calculatepayout',
        //contentType: "application/json; charset=utf-8",
        success: function (data) {
            //alert(data);
            var result = $.parseJSON(data);
            try{ $('#totalAmount').html(addCommas(result.data.Payout.toFixed(2)) + ' ' + sesi_currency);
                $('#totalCoupons').html(result.data.CouponCount);
            } catch (ex){ $('#totalAmount').html('0,00'); $('#totalCoupons').html('0');}
        }
    });
}
...
function fnConfirmCoupon() {
    ...
    if ($('#totalAmount').has('.dot_anim').length == 0 && ($('#totalAmount').html()) != '0,00 TRY' && ($('#totalAmount').html()) != '0,00' && $('.cBet').length > 0) {
        $('body').css({'overflow': 'hidden'});
        var checkboxvalues = $('input[name="edtSystem"]:checked').map(function () {
            return this.value;
        }).get();
        $.ajax({
            type: 'POST',
            data: 'oran=' + $('.oran-rakam').html() + '&mac_sayisi=' + $('.cBet').length + '&totalCoupons=' + $('#totalCoupons').html() + '&totalAmount=' + $('#totalAmount').html() + '&edtAmount=' + $('#edtAmount').val() + '&edtSystem=' + checkboxvalues.join(','),
            url: '/confirmcoupon',
            success: function (data) {
...
function fnSaveCoupon() {
    $('body').append('<div id="loadoverlay_coupon" style="position:fixed; top:0px; left:0px; bottom:0px; right:0px; width:100%; height:100%; background:url(/images/darkoverlay.png); text-align:center; padding-top:200px; z-index:10000;"><img alt="" src="/images/spinner.gif" /></div>');
    var checkboxvalues = $('input[name="edtSystem"]:checked').map(function () { return this.value; }).get();
    if ($('#edtAcceptChanges').is(":checked")) { var pAcceptChanges = "TRUE"; } else { var pAcceptChanges = "FALSE"; }
    $.ajax({
        type: 'POST',
        data: 'edtAmount=' + $('#edtAmount').val() + '&edtSystem=' + checkboxvalues.join(',') + '&edtAcceptChanges=' + pAcceptChanges,
        url: '/savecoupon',
        success: function (data) {
            ...
            setTimeout(function () { $('body').children('#loadoverlay_coupon').remove(); }, 250);
        }
    });
}
...
function sendmessage(message_to_error, message_text_error) {
    $('#btn_send_message').css({'background': 'url(/messageimages/sendpush.png)'});
    if ($('#message_to').val() == '') {
        toastr.options = {"progressBar": true, "closeButton": true, "positionClass": "toast-top-right"};
        ...
        $.ajax({
            type: 'POST',
            data: 'tocustomerid=' + $('#message_to').val() + '&messagetext=' + $('#message_text').val(),
            url: '/sendmessage',
            success: function (data) {
                var json = $.parseJSON(data);
                if (json.vResult == 'TRUE') {
...
function fnGetMessageCount() {
    $.ajax({
        ...
        success: function (data) {
            var json = $.parseJSON(data);
            if (json.result == true) {
                if(json.data != 0) { $('#message_envelope').children('img').addClass('messegae_blink').attr('src', '/icons/envelope_orange.png'); $('#message_envelope') }
                else $('#message_envelope').children('img').removeClass('messegae_blink').attr('src', '/icons/envelope_green.png');
            }
            else {
                $('#message_envelope').children('img').removeClass('messegae_blink').attr('src', '/icons/envelope_green.png');
            }
        }
    });
}
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (json.vResult == 'TRUE') {
                ...
                Ply.dialog("alert",{ effect: "3d-sign" }, "Üyelik İşleminiz Tamamlanmıştır. Giriş yapılıyor.");
...
function fnForgotPass() {
    ...
    $.ajax({
        type: 'POST',
        data: 'edtCaptcha=' + $('#edtForgotCaptcha').val() + '&' +
        'edtEmail=' + $('#edtForgotEmail').val(),
        url: '/sendpass',
        success: function (data) {
            ...
            if (json.vResult == 'TRUE') {
                ...
                $('#forgotPasswordContainer').empty().html('<p>' + json.vContent + '</p>');
                setTimeout(function () {
                    fnForgotPassClose();
                }, 10000);
            } else {
...
function savecouponnew2(){
	...
	if($('.kmiktar').val() && $('.kmiktar').val()!="0.00"){
		...
		$.ajax({
            ...
            data: 'miktar=' + $(".kmiktar").val() + '&retain_selection=' + $("#retain_selection:checked").val(),
			type:"post",
			success:function(data){
				if(data=="ok"){
					fnClearCoupon();
					...
					fnGetBalance();
					Ply.dialog("alert",{ effect: "3d-sign" }, "Kuponunuz Başarıyla Yatırıldı.");
    ...
    $.ajax({
        ...
        success: function(c) {
            if (c.error) {
                ...
                window.open( c.URL ,"LiveCasino","location=1,status=1,scrollbars=1,width=1024,height=836");
            }
        }
    });
}
