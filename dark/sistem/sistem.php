...
//			$odd_2 = $_POST['odd_2'];
//
//			if($db->query("INSERT INTO manual_matches (match_id, sport_id, country_id, league_id, home_team, away_team, match_result, goal_detail, match_datetime) VALUES ('$match_id', '$sport_type', '$country', '$league', '$home_team', '$away_team', '$match_result', '$goal_detail', '$match_datetime')")) {
//				// Ekleme başarılı ise oranları da ekleyin
...
//		}
		...
		$sports = $db->query("SELECT * FROM dark_sports WHERE live = $type ORDER BY listindex ASC");
		$ulkeler = $db->query("SELECT * FROM dark_country WHERE live = $type ORDER BY listindex ASC");
		$ligler = $db->query("SELECT * FROM dark_leagues WHERE live = $type ORDER BY listindex ASC");
